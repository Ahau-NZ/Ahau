      ...
      <template v-slot:activator="{ on, attrs }">
          <v-row :class="margin" justify="start">
            <v-col cols="12" md="auto" class="pa-0 pb-5">
              <v-card-text class="text-caption py-0 text-md-right">Choose who has access to this {{ accessString }}:</v-card-text>
              <v-btn
                v-bind="attrs"
                v-on="on"
                text
              ...
              <v-card-text v-if="access && !disabled" class="font-italic font-weight-light text-caption py-0 text-md-right">
                {{ access.isPersonalGroup ? `Only you have access to this ${accessString}` : `Only ${access.preferredName} will have access to this story` }}
              </v-card-text>

              <v-card-text v-if="access && disabled" class="font-italic font-weight-light text-caption py-0 text-md-right">
                {{ access.isPersonalGroup ? `Only you have access to this ${accessString}` : `Only ${access.preferredName} has access to this story` }}
              </v-card-text>
            </v-col>
          </v-row>
      </template>
