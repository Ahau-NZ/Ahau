<template>
  <div id="app">
        ...
        <svg :width="settings.width" :height="settings.height" style="{margin-left: 100px;}">
          ...
          <transition-group tag="g" name="node">
            <g class="node" v-for="node in nodes" :key="node.id" :style="node.style">
              <circle class="node-circle" @contextmenu.prevent="$refs.menu.open" :r="node.radius" />
            </g>
          </transition-group>
        </svg>
      </v-row>
    ...
    <vue-context ref="menu">
      <li>
        <a href="#" @click.prevent="onClick('Option1')">Option 1</a>
      </li>
      <li>
        <a href="#" @click.prevent="onClick('Option2')">Option 2</a>
      </li>
    </vue-context>
  </div>
...
export default {
  ...
  computed: {
    nodes () {
      ...
      const treeLayout = d3
        .tree() // create a new tree layout and specify the size
        .size([250, 200])
      ...
      return output.map((d, i) => {
        // defined my own mapping for each node
        return {
          id: 'node-' + d.data.id,
          name: d.data.name, // name of the person in this instance
          index: i,
          style: {
            transform: `translate(${d.x + 100}px, ${d.y + 100}px)`
    ...
    links () {
      ...
      return output.map((d, i) => {
        // defined my own mapping for each link
        return {
